...
if __name__ == "__main__":
    ...
    ax.scatter(xs[:, 0], xs[:, 1], xs[:, 2], s = 1, c = ys)
    ax.set_title('origin')
    ...
    with trange(30) as t:
        for i in t:
            ...
            for x, y in trainloader:
                ...
                kl1 = KL(mu_w, logvar_w, torch.zeros_like(mu_w), torch.ones_like(logvar_w) * np.log(0.1))
    ...
    ax.scatter(pred_x[:, 0], pred_x[:, 1], pred_x[:, 2], s = 1, c = ys)
